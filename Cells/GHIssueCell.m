...
static NSArray *GHValidLabels(NSDictionary *issue) {
    NSMutableArray *validLabels = [NSMutableArray array];
    NSArray *rawLabels = [issue[@"labels"] isKindOfClass:[NSArray class]] ? issue[@"labels"] : nil;
...
+ (CGFloat)heightForIssue:(NSDictionary *)issue width:(CGFloat)width {
    ...
    NSString *title = GHSafeString(issue, @"title");
    ...
    NSArray *validLabels = GHValidLabels(issue);
...
- (void)configureWithIssue:(NSDictionary *)issue {
    NSDictionary *user = [issue[@"user"] isKindOfClass:[NSDictionary class]] ? issue[@"user"] : nil;
    NSString *title = GHSafeString(issue, @"title");
    NSString *authorLogin = GHSafeString(user, @"login");
    NSNumber *number = [issue[@"number"] isKindOfClass:[NSNumber class]] ? issue[@"number"] : @0;
    NSNumber *comments = [issue[@"comments"] isKindOfClass:[NSNumber class]] ? issue[@"comments"] : @0;
    NSString *state = GHSafeString(issue, @"state");
    NSString *stateReason = GHSafeString(issue, @"state_reason");
    NSString *createdAt = GHSafeString(issue, @"created_at");

    self.titleLabel.text = title.length > 0 ? title : GHL(@"(без заголовка)");
    self.titleLabel.textColor = GHPrimaryTextColor();

    UIColor *stateColor;
    if ([state isEqualToString:@"closed"]) {
        stateColor = (stateReason.length == 0 || [stateReason isEqualToString:@"completed"])
            ? [UIColor colorWithRed:0.51 green:0.31 blue:0.87 alpha:1.0]
